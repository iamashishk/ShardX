...
export async function resolveAutoFields(
  ...
  if (proxy) {
    try { geo = await geoCheckVia(proxy,"ip-api.com",directProvider); } catch { geo = null; }
  }
  if (!geo) {
    try { geo = await geoCheckVia(null,"ip-api.com",directProvider); } catch { geo = null; }
  }
