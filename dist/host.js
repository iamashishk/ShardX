...
export function hostLogicalCores() {
    try {
        const n = cpus().length;
        return n > 0 ? n : 8;
    }
...
export function hostRamGb() {
    ...
    try {
        if (plat === "darwin") {
            const out = execFileSync("sysctl", ["-n", "hw.memsize"], { stdio: ["ignore", "pipe", "ignore"], timeout: 2000 }).toString().trim();
        ...
        if (plat === "win32") {
            const out = execSync('powershell -NoProfile -Command "(Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory"', { stdio: ["ignore", "pipe", "ignore"], timeout: 4000 }).toString().trim();
            const bytes = Number(out);
            if (!Number.isFinite(bytes) || bytes <= 0)
                return null;
            return Math.floor(bytes / (1024 ** 3));
...
export function hostRamBucketGb() {
    const gb = hostRamGb();
    if (gb === null)
        return 16;
    if (gb >= 32)
...
export function hostScreenSize() {
    const plat = osPlatform();
    try {
        if (plat === "darwin")
            return macScreenSize();
        if (plat === "win32")
            return windowsScreenSize();
        if (plat === "linux")
            return linuxScreenSize();
    }
    catch {
        return null;
    }
...
function macScreenSize() {
    ...
    let out;
    try {
        out = execFileSync("system_profiler", ["SPDisplaysDataType"], { stdio: ["ignore", "pipe", "ignore"], timeout: 4000 }).toString();
    }
    catch {
        return null;
    }
...
function windowsScreenSize() {
    try {
        const out = execSync('powershell -NoProfile -Command ' +
            '"Add-Type -AssemblyName System.Windows.Forms; ' +
            '$s=[System.Windows.Forms.Screen]::PrimaryScreen.Bounds; ' +
            '\\"$($s.Width)x$($s.Height)\\""', { stdio: ["ignore", "pipe", "ignore"], timeout: 4000 }).toString().trim();
...
function linuxScreenSize() {
    let out;
    try {
        out = execFileSync("xrandr", ["--query"], { stdio: ["ignore", "pipe", "ignore"], timeout: 4000 }).toString();
    }
    catch {
        return null;
    }
