...
export function defaultCacheDir() {
    ...
    if (plat === "darwin")
        return join(homedir(), "Library", "Application Support", "shardx-sdk");
    if (plat === "win32")
        return join(process.env.LOCALAPPDATA ?? homedir(), "shardx-sdk");
...
export const FINGERPRINTS_ARCHIVE = {
...
export class Runtime {
    ...
    _chromiumVersion = CHROMIUM_VERSION;
    ...
    get manifestPath() { return join(this.root, "manifest.json"); }
    get logPath() { return join(this.root, "install.log"); }
    get binaryPath() { return join(this.root, ...this.spec.binarySubpath); }
    get fingerprintsDir() {
    ...
    get installed() { return existsSync(this.binaryPath) || existsSync(this.manifestPath); }
    /** Engine chromium version (manifest-driven; set on install()). */
    get chromiumVersion() { return this._chromiumVersion; }
    /** GREASE brand from the manifest (e.g. "Not)A;Brand"); set on install(). */
    get greaseBrand() { return this._greaseBrand; }
    /** GREASE version from the manifest (e.g. "24"); set on install(). */
    get greaseVersion() { return this._greaseVersion; }
    ...
    installedEngineVersion() {
        try {
            const plat = osPlatform();
            if (plat === "darwin") {
                const versions = join(this.root, "ShardX-Mac-arm64", "ShardX.app", "Contents", "Frameworks", "ShardX Framework.framework", "Versions");
    ...
    effectiveInstalledVersion(local) {
    ...
    async install(opts = {}, attempt = 1) {
        ...
        if (force || !fpDirHasJson || (remoteFp !== undefined && local.fingerprints_etag !== remoteFp)) {
            ...
            if (remoteFp !== undefined)
                local.fingerprints_etag = remoteFp;
        }
    ...
    async downloadAndExtract(arch, dest) {
        const url = `${PUB_BASE}/${arch.key}`;
        mkdirSync(dest, { recursive: true });
        const tmp = join(dest, `.${arch.key}.tmp`);
        const r = await fetch(url);
        if (!r.ok || !r.body)
            throw new Error(`download ${arch.key}: HTTP ${r.status}`);
    ...
    async installFingerprints() {
        ...
        const r = await fetch(url);
        if (!r.ok || !r.body)
            throw new Error(`download fingerprints: HTTP ${r.status}`);
        const total = Number(r.headers.get("content-length") ?? 0);
        let received = 0;
        const reader = r.body.getReader();
        const out = createWriteStream(tmp);
...
function systemUnzip(archive, dest) {
    mkdirSync(dest, { recursive: true });
    ...
    if (r.error) {
        if (r.error.code === "ENOENT") {
            throw new Error("system `unzip` not found — install with `apt install unzip` / `brew install unzip`");
        }
        throw r.error;
    }
    if ((r.status ?? 0) > 1) {
