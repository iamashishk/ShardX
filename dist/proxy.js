...
export async function probeUdp(p, timeoutMs = 6000) {
    if (p.scheme !== "socks5")
        return null;
    ...
    return await new Promise((resolve) => {
        ...
        tcp.on("data", async (chunk) => {
            ...
            if (state === "assoc" && buf.length >= 10) {
                ...
                let stunIdx = 0;
                let currentTxn = Buffer.alloc(0);
                const udpTimer = setTimeout(() => { try {
                    udp.close();
                }
                catch { } done(null); }, timeoutMs);
                const sendNextStun = () => {
                    ...
                    portBuf.writeUInt16BE(stunPort, 0);
                ...
                udp.on("error", () => { clearTimeout(udpTimer); try {
                    udp.close();
                }
                catch { } done(null); });
                udp.on("message", (msg) => {
                    ...
                    if (offset < 0 || msg.length < offset + 20) {
                        sendNextStun();
                        return;
                    }
