...
export const MACOS_PLATFORM_VERSIONS = [
    "14.6.1", "14.7", "14.7.1", "14.7.2",
    "15.4", "15.4.1", "15.5", "15.6", "15.6.1", "15.7",
    "26.0", "26.0.1", "26.1",
];
export const WINDOWS_PLATFORM_VERSIONS = [
    "10.0.0",
    "13.0.0",
...
export const LINUX_PLATFORM_VERSIONS = [
    "5.15.0", "6.1.0", "6.5.0",
    "6.6.0", "6.8.0", "6.10.0", "6.11.0", "6.12.0",
    "6.14.0", "6.15.0", "6.16.0",
];
export const MAC_HW_CONFIGS = {
...
export const X86_CORES = [4, 6, 8, 12, 16, 20, 24, 28, 32];
...
export function randomizePlatformVersion(cfg) {
    const plat = platformOf(cfg);
    const pool = plat === "macOS" ? MACOS_PLATFORM_VERSIONS :
 ...
 */
export function randomizeHardware(cfg, profileId) {
    const plat = platformOf(cfg);
    let cores;
    let mem;
    if (plat === "macOS" && profileId && profileId in MAC_HW_CONFIGS) {
        ...
        if (candidates.length > 0) {
            ...
            cores = X86_CORES.reduce((best, n) => Math.abs(n - c) < Math.abs(best - c) ? n : best, X86_CORES[0]);
        }
